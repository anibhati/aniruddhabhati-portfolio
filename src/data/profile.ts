import type { ToolGroup, Involvement } from "./types";
export const toolkit: ToolGroup[] = [
  { group: "Languages", items: ["Python", "Java", "TypeScript", "JavaScript", "SQL", "Swift"] },
  { group: "Web + mobile", items: ["React", "Next.js", "Node.js", "Angular", "React Native", "Expo", "Tailwind CSS", "REST APIs"] },
  { group: "AI + ML", items: ["TensorFlow", "Keras", "scikit-learn", "LangGraph", "Ollama", "SBERT", "FAISS"] },
  { group: "Data + infra", items: ["Postgres", "Supabase", "Prisma", "Redis", "Docker", "Databricks", "Vercel", "Git"] },
];

export const involvement: Involvement[] = [
  { name: "Big Data Analytics Association, ML Track", note: "Led the technical build of SignalSpace. 1st Place People's Choice at the Research Gala.", when: "Sep 2025 to now" },
  { name: "Engineering Scholars Leadership Council", note: "Plan professional development and community events for a selective cohort of 100+ engineering students.", when: "2025 to now" },
];
